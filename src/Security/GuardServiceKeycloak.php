...
class GuardServiceKeycloak extends SocialAuthenticator
{
    ...
    public function getUser($credentials, UserProviderInterface $userProvider)
    {
        ...
        if ($existingUser) {
            ...
            $existingUser->getMyUser()->setFirstName($firstName);
            $existingUser->getMyUser()->setLastName($lastName);
        ...
        $user = new User();
        $user->setPassword(md5(uniqid('kjsdjfbjb', true)));
        $user->setEmail($email);

        $myUser = new MyUser();
        $myUser->setCreatedAt(new \DateTime());
        $myUser->setUid(md5(uniqid()));
        $myUser->setUpdatedAt(new \DateTime());
        $myUser->setActive(true);
        $myUser->setDsgvo(false);
