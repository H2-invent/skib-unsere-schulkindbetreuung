...
class OrganisationController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        $organisation = $this->managerRegistry->getRepository(Organisation::class)->findBy(['stadt' => $city, 'deleted' => false]);
    ...
    public function newOrg(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (count($errors) == 0) {
                ...
                return $this->redirectToRoute('city_admin_organisation_show', ['snack' => $text, 'id' => $city->getId()]);
            }
        }
    ...
    public function editOrg(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if (count($errors) == 0) {
                ...
                return $this->redirectToRoute('city_admin_organisation_detail', ['snack' => $text, 'id' => $organisation->getId()]);
            }
        }
        $title = $translator->trans('Organisation anlegen');
    ...
    public function detailSchool(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        return $this->render('cityAdminOrganisation/organisationDetail.html.twig', ['stadt' => $city, 'organisation' => $organisation]);
    }
