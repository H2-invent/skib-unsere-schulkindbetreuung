...
class BlockController extends AbstractController
{
    ...
    public function showSchulen(Request $request)
    {
        ...
        $schule = $this->managerRegistry->getRepository(Schule::class)->findBy(['organisation' => $organisation, 'deleted' => false]);
        if ($organisation != $this->getUser()->getOrganisation()) {
            throw new \Exception('Wrong Organisation');
        }

        return $this->render('block/schulen.html.twig', ['schule' => $schule]);
    }
    ...
    public function showBlocks(Request $request)
    {
        ...
        $activity = $this->managerRegistry->getRepository(Active::class)->findBy(['stadt' => $schule->getStadt()], ['bis' => 'desc']);
        $blocks = $this->managerRegistry->getRepository(Zeitblock::class)->findAll();

        return $this->render('block/blocks.html.twig', ['schuljahre' => $activity, 'schule' => $schule, 'blocks' => $blocks]);
    }
    ...
    public function getBlocks(Request $request)
    {
        ...
        $activity = $this->managerRegistry->getRepository(Active::class)->findOneBy(['id' => $request->get('id'), 'stadt' => $schule->getStadt()]);
        $blocks = $this->managerRegistry->getRepository(Zeitblock::class)->findBy(['schule' => $schule, 'active' => $activity], ['von' => 'asc']);
        ...
        return $this->render('block/blockContent.html.twig', ['blocks' => $renderBlock, 'year' => $activity, 'shool' => $schule]);
    }
    ...
    public function newBlock(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        $form = $this->createForm(BlockType::class, $block, [
            'action' => $this->generateUrl('block_schule_newBlocks', ['year' => $activity->getId(), 'shool' => $shool->getId(), 'weekday' => $block->getWochentag()]),
    ...
    public function editBlock(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        $form = $this->createForm(BlockType::class, $block, [
            'action' => $this->generateUrl('block_schule_editBlocks', ['id' => $block->getId()]),
        ]);
        $form->remove('ganztag')
            ->remove('save')
    ...
    public function linkBlock(Request $request, ValidatorInterface $validator, TranslatorInterface $translator)
    {
        ...
        $blocks1 = $this->managerRegistry->getRepository(Zeitblock::class)->findBy(
            ['schule' => $block->getSchule(),
                ...
                'active' => $block->getActive(), 'deleted' => false], ['von' => 'ASC'], );
        $blocks = [];
        ...
        $form = $this->createForm(BlockAbhangigkeitType::class, $block, [
            'action' => $this->generateUrl('block_schule_linkBlock', ['id' => $block->getId()]),
            'blocks' => $blocks,
        ]);
    ...
    public function duplicateBlock(Request $request, TranslatorInterface $translator)
    {
        $block = $this->managerRegistry->getRepository(Zeitblock::class)->find($request->get('id'));
        if (!$block) {
            return new JsonResponse(['error' => 1, 'snack' => $translator->trans('Block nicht gefunden.')]);
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            if (count($weekdays) === 0) {
                return new JsonResponse(['error' => 1, 'snack' => $translator->trans('Bitte mindestens einen Tag auswählen.')]);
            }
            ...
            return new JsonResponse(['error' => 0, 'snack' => $translator->trans('Block erfolgreich dupliziert.')]);
        }
