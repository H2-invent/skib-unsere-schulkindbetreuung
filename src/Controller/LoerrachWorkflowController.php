...
class LoerrachWorkflowController extends AbstractController
{
        ...
        SchuljahrService $schuljahrService)
    {
        ...
        if ($schuljahr === null) {
            return $this->redirectToRoute('workflow_closed', ['slug' => $stadt->getSlug()]);
        }
        if ($parameterBag->get('wartung') == 'true') {
            return $this->redirectToRoute('workflow_wartung', ['redirect' => $this->generateUrl('loerrach_workflow_adresse', ['slug' => $stadt->getSlug()])]);
        }
        ...
        $formArr = ['einkommen' => array_flip($stadt->getGehaltsklassen()), 'beruflicheSituation' => $this->beruflicheSituation, 'stadt' => $stadt];
        ...
        if ($request->cookies->get('KindID')) {
            return $this->redirectToRoute('loerrach_workflow_schulen', ['slug' => $stadt->getSlug()]);
        }
        ...
        if ($form->isSubmitted()) {
            ...
            if (count($errors) == 0) {
                ...
                $response = $this->redirectToRoute('workflow_confirm_Email', ['redirect' => $this->generateUrl('loerrach_workflow_schulen', ['slug' => $stadt->getSlug()], UrlGeneratorInterface::ABSOLUTE_URL), 'uid' => $adresse->getUid(), 'stadt' => $stadt->getId()]);
        ...
        return $this->render('workflow/loerrach/adresse.html.twig', ['title' => $title, 'stadt' => $stadt, 'form' => $form, 'errors' => $errorsString]);
    }

    #[Route(path: '/{slug}/schulen', name: 'loerrach_workflow_schulen', methods: ['GET'])]
    public function schulenAction(#[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, StamdatenFromCookie $stamdatenFromCookie, SchuljahrService $schuljahrService)
    {
        ...
        if ($stamdatenFromCookie->getStammdatenFromCookie($request)) {
            ...
            return $this->redirectToRoute('loerrach_workflow_adresse', ['slug' => $stadt->getSlug()]);
        }
        $kinder = [];
        if ($request->cookies->get('KindID')) {
        ...
        return $this->render('workflow/loerrach/schulen.html.twig', ['isEdit' => $isEdit, 'schule' => $schule, 'stadt' => $stadt, 'adresse' => $adresse, 'kinder' => $renderKinder]);
    }

    #[Route(path: '/{slug}/schulen/kind/neu', name: 'loerrach_workflow_schulen_kind_neu', methods: ['GET', 'POST'])]
    public function neukindAction(SchulkindBetreuungKindNeuService $schulkindBetreuungKindNeuService, #[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, ValidatorInterface $validator, TranslatorInterface $translator, StamdatenFromCookie $stamdatenFromCookie, SchuljahrService $schuljahrService, ParameterBagInterface $parameterBag)
    {
        $adresse = new Stammdaten();
        ...
        $form = $this->createForm(LoerrachKind::class, $kind, ['schuljahr' => $schuljahr, 'validation_groups' => ['Eltern'], 'action' => $this->generateUrl('loerrach_workflow_schulen_kind_neu', ['slug' => $stadt->getSlug(), 'schule_id' => $schule->getId()])]);
        ...
        try {
            ...
            return new JsonResponse(['snack' => [['type' => 'error', 'text' => $text]]]);
        }
        ...
        return $this->render('workflow/loerrach/kindForm.html.twig', ['schule' => $schule, 'form' => $form->createView()]);
    }

    #[Route(path: '/{slug}/schulen/kind/edit', name: 'loerrach_workflow_schulen_kind_edit', methods: ['GET', 'POST'])]
    public function editkindAction(SchulkindBetreuungKindNeuService $schulkindBetreuungKindNeuService, #[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, ValidatorInterface $validator, TranslatorInterface $translator, StamdatenFromCookie $stamdatenFromCookie, SchuljahrService $schuljahrService)
    {
        $adresse = new Stammdaten();
        ...
        $form = $this->createForm(LoerrachKind::class, $kind, [
            'schuljahr' => $schuljahr, 'action' => $this->generateUrl('loerrach_workflow_schulen_kind_edit', ['slug' => $stadt->getSlug(), 'kind_id' => $kind->getId()]),
        ]);
        $form->remove('art');
        ...
        return $this->render('workflow/loerrach/kindForm.html.twig', ['form' => $form]);
    }

    #[Route(path: '/{slug}/schulen/kind/delete', name: 'loerrach_workflow_kind_delete', methods: ['DELETE'])]
    public function deleteAction(#[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, ValidatorInterface $validator, StamdatenFromCookie $stamdatenFromCookie)
    {
        // Include Parents in this route
        $adresse = new Stammdaten();
        if ($stamdatenFromCookie->getStammdatenFromCookie($request)) {
        ...
        return new JsonResponse(['redirect' => $this->generateUrl('loerrach_workflow_schulen', ['slug' => $stadt->getSlug()])]);
    }
    ...
    public function kindzeitblockAction(#[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, StamdatenFromCookie $stamdatenFromCookie, SchuljahrService $schuljahrService)
    {
        $adresse = new Stammdaten();
        ...
        if ($kind->getArt() == 1) {
            $req['ganztag'] = 0;
            $block = $this->managerRegistry->getRepository(Zeitblock::class)->findBy($req, ['von' => 'asc']);
            $req['ganztag'] = $kind->getArt();
            $block = array_merge($block, $this->managerRegistry->getRepository(Zeitblock::class)->findBy($req, ['von' => 'asc']));
        ...
        return $this->render('workflow/loerrach/blockKinder.html.twig', ['stadt' => $stadt, 'kind' => $kind, 'blocks' => $renderBlocks, 'schuljahr' => $schuljahr]);
    }

    #[Route(path: '/{slug}/kinder/block/toggle', name: 'loerrach_workflow_kinder_block_toggle', methods: ['PATCH'])]
    public function kinderblocktoggleAction(#[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, ValidatorInterface $validator, TranslatorInterface $translator, StamdatenFromCookie $stamdatenFromCookie, ToogleKindBlockSchulkind $toogleKindBlockSchulkind)
    {
        // Include Parents in this route
        $adresse = new Stammdaten();
        if ($stamdatenFromCookie->getStammdatenFromCookie($request)) {
        ...
        $kind = $this->managerRegistry->getRepository(Kind::class)->findOneBy(['eltern' => $adresse, 'id' => $request->get('kinder_id')]);
        ...
        if ($block->getDeaktiviert() || $block->getDirektbuchungDeaktiviert()) {
            return new JsonResponse(['error' => 1, 'snack' => 'Error, this action is not allowed']);
        }
    ...
    public function mittagessenAction(Request $request, #[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, StamdatenFromCookie $stamdatenFromCookie)
    {
        $adresse = new Stammdaten();
        ...
        return $this->render('workflow/loerrach/mittagessen.html.twig', ['stadt' => $stadt, 'schule' => $renderSchulen]);
    }

    #[Route(path: '/{slug}/bezahlen', name: 'loerrach_workflow_bezahlen')]
    public function sepaAction(ErrorService $errorService, SchulkindBetreuungKindSEPAService $schulkindBetreuungKindSEPAService, Request $request, #[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, StamdatenFromCookie $stamdatenFromCookie, ValidatorInterface $validator)
    {
        $adresse = new Stammdaten();
        ...
        if ($stadt->getSkibSettingsBypassBankdaten()) {
            $response = $this->redirectToRoute('loerrach_workflow_zusammenfassung', ['slug' => $stadt->getSlug()]);
        ...
        return $this->render('workflow/loerrach/bezahlen.html.twig', ['stadt' => $stadt, 'form' => $form, 'errors' => $errorString, 'organisation' => $renderOrganisation]);
    }
        ...
        BerechnungsService $berechnungsService)
    {
        ...
        return $this->render('workflow/loerrach/zusammenfassung.html.twig', [
        ...
        ElternService $elternService)
    {
        ...
        if ($request->cookies->get('KindID')) {
            $cookie_kind = explode('.', $request->cookies->get('KindID'));
            $kindeToEdit = $this->managerRegistry->getRepository(Kind::class)->findOneBy(['id' => $cookie_kind[0]]);
            $isEdit = true;
        }
        ...
        foreach ($kinder as $data) {
            if ($data->getTageWithBlocks() < $stadt->getMinDaysperWeek() && !$isEdit) {
                $this->redirectToRoute('loerrach_workflow_zusammenfassung', ['slug' => $stadt->getSlug()]);
            }
        }
        ...
        $response = $this->render('workflow/abschluss.html.twig', ['kind' => $kinder, 'eltern' => $adresse, 'stadt' => $stadt]);
    ...
    public function berechnungAction(#[MapEntity(mapping: ['slug' => 'slug'])] Stadt $stadt, Request $request, ValidatorInterface $validator, TranslatorInterface $translator, StamdatenFromCookie $stamdatenFromCookie, BerechnungsService $berechnungsService)
    {
        // Load the data from the city into the controller as $stadt
        $result = [
            'error' => 0,
        ...
        $kind = $this->managerRegistry->getRepository(Kind::class)->findOneBy(
            ['id' => $request->get('kind_id'), 'eltern' => $adresse]
        );
    ...
    public function prinPdf(Request $request, ValidatorInterface $validator, TranslatorInterface $translator, PrintService $print, StamdatenFromCookie $stamdatenFromCookie)
    {
        $elter = $stamdatenFromCookie->getStammdatenFromCookie($request);
        $stadt = $elter->getKinds()[0]->getSchule()->getStadt();

        $kind = $this->managerRegistry->getRepository(Kind::class)->findOneBy(['eltern' => $elter, 'id' => $request->get(
            'id')]);
