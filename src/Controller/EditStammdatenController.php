...
class EditStammdatenController extends AbstractController
{
    ...
    public function index(Request $request, TranslatorInterface $translator): Response
    {
        ...
        if (($form->isSubmitted() && $form->isValid()) || $this->getUser()->getOrganisation()->getStadt()->getNoSecCodeForChangeChilds()) {
            ...
            if ($input['seccode'] == $adresseTmp->getSecCode() || $this->getUser()->getOrganisation()->getStadt()->getNoSecCodeForChangeChilds()) {
                return $this->redirectToRoute('edit_stammdaten_edit', ['eltern_id' => $adresse->getId()]);
            }
    ...
    public function edit(
        ...
        $formArr = ['einkommen' => array_flip($stadt->getGehaltsklassen()), 'beruflicheSituation' => $loerrachWorkflowController->beruflicheSituation, 'stadt' => $stadt];
        ...
        if ($form->isSubmitted()) {
            ...
            if (count($errors) == 0) {
                ...
                return $this->redirectToRoute('child_show', ['id' => $this->getUser()->getOrganisation()->getId(), 'snack' => $translator->trans('Stammdaten gespeichert')]);
            }
        }
