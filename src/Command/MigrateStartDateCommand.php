    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
            ...
            if ($schuljahr) {
                ...
                if ($kind->getSaved()) {
                    ...
                    if ($kind->getHistory() === 0) {
                        ...
                        if ($eltern->getCreatedAt() > $schuljahr->getVon()) {
                            $history = $this->em->getRepository(Kind::class)->findOneBy(['tracing' => $kind->getTracing(), 'history' => $kind->getHistory() - 1]);
        ...
        foreach ($kinder as $kind) {
            ...
            if (!in_array($tracing, $checked)) {
                $allKindsWithTracing = $this->em->getRepository(Kind::class)->findBy(['tracing' => $tracing]);
                $checked[] = $tracing;
                $deleted = true;
