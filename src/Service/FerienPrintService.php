...
class FerienPrintService
{
    ...
    public function printPdfTicket($fileName, KindFerienblock $ferienblock, $type = 'D')
    {
        ...
        $code = $this->router->generate('ferien_checkin', ['checkinID' => $ferienblock->getCheckinID()], UrlGeneratorInterface::ABSOLUTE_URL);
        $pdf->write2DBarcode($code, 'QRCODE,Q', 139, 39, 45, 45, $style, 'N');

        $kindData = $this->templating->render('ferien_ticket/index.html.twig', ['kind' => $kind, 'organisation' => $organisation, 'ferienblock' => $ferienblock]);
        $pdf->writeHTMLCell(
            0,
            0,
            20,
